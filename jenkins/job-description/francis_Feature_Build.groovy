...
def customLib = library identifier: 'francis_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kobbyopoku/francis_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.Francis.new()
    ...
    stage('Prepare') {
      steps {
        ...
        script {
          ...
          jslGit.checkout( config, "kobbyopoku", "francis", branch_name)
          }
        }
      }
stage('Build') {
...
"""
def job = pipelineJob("francis_Feature_Build");
...
job.with {
    ...
    authenticationToken('AdgSMbcjGKd7vL3FxICKhOGCqPV2YYSf')
